bmApp.factory('BookDataService', function($http){
	...
	srv.getBookByIsbn = function(isbn){
		return $http.get(srv._baseUrl + '/api/books/' + isbn);
	};
	
	srv.getBooks = function(){
		return $http.get(srv._baseUrl + '/api/books');
	};

	srv.getTags = function(){
		return $http.get(srv._baseUrl + '/api/tags');
	};
	...
	srv.updateBook = function(book){
		return $http.put(srv._baseUrl + '/api/books/' + book.isbn, book);
	};
	
	srv.deleteBookByIsbn = function(isbn){
		return $http.delete(srv._baseUrl + '/api/books/' + isbn);
	};
